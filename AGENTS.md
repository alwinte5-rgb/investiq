...
| Hosting | **Railway** |
| Database | **Railway Postgres plugin** + Prisma 7 |
| Payments | **Stripe** |
...
### Never put prefers-color-scheme inside an SVG asset
The rule travels with the file, so the graphic follows the viewer's OS theme rather than the background it is placed on — a logo on a white header vanishes for every visitor in dark mode. Use fill=currentColor and let the page decide, or ship an explicit light and dark pair.

### Name the account after the product, not the project that made it
A dashboard name and a statement descriptor are customer-facing. A retired internal brand there is confusing at best and a chargeback at worst.

### Compose dates from parts, never from a locale format string
`Intl.DateTimeFormat.format` picks its separator from whatever ICU version the runtime was built with, so a server and a browser can disagree — 'Aug 7, 1:37 PM' against 'Aug 7 at 1:37 PM'. That is a hydration mismatch and React throws the tree away. Use `formatToParts` and join it yourself.

### Prose is not a call site
Counting a mention in a README or a comment as a use means naming a function in documentation silently disables the check that verifies it is called. Search code, skip markdown.

### Reading a secret from the environment is correct, not a leak
`api_secret: process.env.CLOUDINARY_API_SECRET` is the right way to hold a credential. A scanner that cannot tell it from a hardcoded literal reports every properly-written config as a breach, and a checker that cries wolf gets switched off.
