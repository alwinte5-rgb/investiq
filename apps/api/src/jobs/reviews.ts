...
/**
 * Scheduled review job (Layer 4). Invoked by a Railway cron. Two modes:
 *   npm run -w @investiq/api job:reviews -- AUTO     (recommended: one daily cron)
 *   npm run -w @investiq/api job:reviews -- MORNING  (a single explicit period)
 * AUTO expands to MORNING daily, WEEKLY on Mondays, MONTHLY on the 1st — so one
 * daily schedule covers all three cadences. Generates each eligible user's
 * review once per period (idempotent) and delivers it honoring channel
 * preferences + quiet hours. Safe to re-run: the (userId, period, periodKey)
 * constraint and per-channel dedupe prevent duplicate reviews or double sends.
 */

const PERIODS: ReviewPeriod[] = ["MORNING", "WEEKLY", "MONTHLY"];

/** Resolve the CLI arg into the list of periods to run this invocation. */
function resolvePeriods(argv: string[], now: Date): ReviewPeriod[] {
  const raw = (argv[2] ?? "AUTO").toUpperCase();
  if (raw === "AUTO") return autoReviewPeriods(now);
  if (!PERIODS.includes(raw as ReviewPeriod)) {
    throw new Error(`Invalid period "${raw}". Use AUTO or one of: ${PERIODS.join(", ")}`);
  }
...
async function main() {
  ...
  const now = new Date();
  const periods = resolvePeriods(process.argv, now);
  ...
  const stats = { generated: 0, existing: 0, insufficient: 0, delivered: 0, failed: 0, skipped: 0 };
  console.log(
    `[reviews] periods=${periods.join(",")} eligible=${users.length} email=${deps.email.enabled} push=${deps.push.enabled}`,
  );
  ...
  for (const period of periods) {
    for (const u of users) {
      try {
        ...
        stats.failed += 1;
        console.error(`[reviews] period=${period} user=${u.id} error:`, err instanceof Error ? err.message : err);
      }
    }
  }
