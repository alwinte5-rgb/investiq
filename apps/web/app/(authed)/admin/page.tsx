...
export default async function AdminPage() {
  ...
  if (denied) {
    return (
      <div className="rounded-md border border-warn bg-warn-soft p-4 text-sm text-warn">
      ...
      <section className="space-y-2">
        ...
        <div className="overflow-x-auto rounded-md border">
          <table className="w-full text-sm">
            <thead className="bg-raised text-left text-xs text-t3">
              <tr>
            ...
            <tbody className="divide-y">
              {users.map((u) => (
                <tr key={u.id}>
                  ...
                  <td className="px-3 py-2 text-t3">{u._count.watchlists}</td>
              ...
              {users.length === 0 && (
                <tr>
                  <td colSpan={3} className="px-3 py-4 text-center text-t3">
                    No users.
                  </td>
                </tr>
              )}
